 ...
 */
public class OrderDealTwo extends Activity implements OnClickListener{
	
	private static final String ORDER_STATUS = "3";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.list_two);
	...
	private void executeRefreshOrderTask() {
		refreshOrderInfoTask = new com.zrh.urestaurantprivate.thread.RefreshOrderInfoTask(OrderDealTwo.this, refreshListView,ORDER_STATUS);
	...
	private List<OrderInfo> listPrepareInfo = new ArrayList<OrderInfo>();// 预定订单部分
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			if (msg.what == 0x111) {
				if (listPrepareInfo.size() > 0) {
					linearDeal.setVisibility(0);
					linear.setVisibility(8);
					initPrepareOrder();
				} else {
					linear.setVisibility(0);
					linearDeal.setVisibility(8);
				}
				closeReflashDialog();
				...
				closeReflashDialog();
				Toast.makeText(OrderDealTwo.this, "获取数据失败，请检查网络", 1).show();
				linear.setVisibility(0);
				linearDeal.setVisibility(8);
			}
		};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		lv_deal.setDivider(null);
		linear = (LinearLayout) findViewById(R.id.nodata_deal);
		linearDeal = (LinearLayout) findViewById(R.id.ll_deal);
		getInfo();
		tv_back = (TextView) findViewById(R.id.tv_back);
		tv_back.setOnClickListener(this);
	}
	...
	*//**
	 ...
	 *//*
	private void getInfo() {
		oInfoDB = new OrderInfoDB(OrderDealTwo.this);
		listPrepareInfo = oInfoDB.getInfoByOrder_status("1");
		oInfoDB.close();
	...
	*//**
	...
	private void initPrepareOrder() {
		...
		lv_deal.setAdapter(new OrderListAdapter(listPrepareInfo,
				OrderDealTwo.this));
		lv_deal.setOnItemClickListener(this);
	}
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		// TODO Auto-generated method stub
		Intent intent = new Intent(OrderDealTwo.this, OrderDetails.class);
		// listMenuDetails = JsonTools.getMenuData(jsonObjectList
		// .get(jsonObjectList.size() - 1 - arg2));
		oDetailsDb = new OrderDetailsDB(OrderDealTwo.this);
